...
## Kano Agent（說明中心 AI 助手）

- **入口位置：**
  - 說明中心 `help.kano.site` 右下角的 Kano Agent 浮動視窗。
  - Kano 後台右下角（主產品內）也可呼叫同一組 Agent。
- **回答範圍：** 以「操作流程與常見問題」為主（登入、建站、編輯器、SEO、追蹤、網域等），答案完全來自 Kano 說明中心文件，不連線到客戶的帳號資料。
- **內容來源：** 以 RAG 方式從已部署的說明中心頁面擷取相關段落，再由 LLM 生成答案。工程細節與 chunk 策略記錄於 `kano-help-center/docs/rag-and-agent.md`。
- **維運重點：**
  - 文件更新後需重新部署說明中心，並觸發索引重建（CI 已支援 post-deploy ingest，亦可手動呼叫）。
  - FAQ 與 troubleshooting 區塊須維持「一問一答」與 H2 結構，才能保證 Agent 檢索品質。
